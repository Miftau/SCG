    ...
    <div className="w-full bg-white shadow-md rounded-lg p-4 sm:p-6">
      ...
      >
        {questions.map((item, i) => (
          ...
          >
            <AccordionTrigger
              className="group flex flex-row justify-between items-center py-8 text-lg md:text-xl font-bold text-blue-600 hover:no-underline"
            >
              <span className="leading-relaxed">{item.q}</span>
              {/* أيقونة + */}
            </AccordionTrigger>
            <AccordionContent className="text-gray-600 text-base leading-relaxed py-8">
              {item.a}
            </AccordionContent>
          </AccordionItem>
        ))}
